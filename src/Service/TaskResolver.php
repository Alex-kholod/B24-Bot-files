...
final class TaskResolver
{
    ...
    /**
     * Остаточная гонка (намеренно не устраняется): вебхук и тик cron, обрабатывающие два РАЗНЫХ
     * файлов одного и того же клиента, впервые встреченного системой, могут одновременно не найти
     * закешированную задачу в TaskLinkRepository и оба вызвать addTask(). Аренда строк очереди
     * (PendingFileRepository::claim) не спасает — она защищает одну строку, а не клиента в целом.
     * Устранение потребовало бы блокировки по клиенту, что несоразмерно для одного портала с
     * низкой нагрузкой: последствие — лишняя задача, а не потеря документа, что и есть тот
     * инвариант, который система обязана сохранять.
     */
